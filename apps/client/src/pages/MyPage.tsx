...
import type { RepositoryResponse } from '@leita/types';
...
const MyPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'history' | 'qna'>(() => {
    if (tabParam && ['profile', 'history', 'qna'].includes(tabParam)) {
      return tabParam as 'profile' | 'history' | 'qna';
    }
    return 'profile';
  });
  ...
  // Q&A Tab States
  const [myQnas, setMyQnas] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (tabParam && ['profile', 'history', 'qna'].includes(tabParam)) {
      setActiveTab(tabParam as 'profile' | 'history' | 'qna');
    } else {
      setActiveTab('profile');
    }
  }, [tabParam]);

  const handleTabChange = (tab: 'profile' | 'history' | 'qna') => {
  ...
  const fetchMyQnas = async (page = 0) => {
    try {
      setQnaLoading(true);
      const res = await qnaApi.getMyQnas(page, 10);
      setMyQnas(res.content || []);
      setQnaTotalPages(res.totalPages || 1);
      setQnaPage(page);
    } catch (err) {
      Logger.error('Failed to fetch QnAs', err);
    } finally {
      setQnaLoading(false);
    }
  };
              ...
              >
                ...
                <div className="p-4">
                  {qnaLoading ? (
                    <div className="py-20 flex flex-col items-center justify-center">
                       <div className="w-10 h-10 border-4 border-[#CAFE33]/20 border-t-[#CAFE33] rounded-full animate-spin mb-4" />
                       <p className="text-gray-500 font-bold">Q&A 내역을 가져오고 있습니다...</p>
                    </div>
                  ) : myQnas.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="w-full border-collapse">
                        <thead>
                          <tr className="text-left border-b border-white/10">
                            <th className="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest">번호</th>
                            <th className="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest">제목</th>
                            <th className="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest">상태</th>
                            <th className="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest text-right">작성 일시</th>
                          </tr>
                        </thead>
                        <tbody>
                          {myQnas.map((qna) => (
                            ...
                            >
                              ...
                              <td className="px-6 py-5 text-right text-xs text-gray-400 group-hover:text-[#CAFE33] transition-colors">
                      ...
                      {qnaTotalPages > 1 && (
                        <div className="mt-8 mb-4">
                          <Pagination
                            ...
                            onPageChange={(page) => fetchMyQnas(page)}
                          />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="py-20 text-center">
                       <p className="text-gray-500 font-bold">작성한 Q&A 문의가 없습니다.</p>
                    </div>
                  )}
                </div>
              </motion.div>
