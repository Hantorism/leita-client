...
import { Header, Footer, StartAttendanceModal, AddAssignmentModal, Button } from '@components';
...
const StudySessionDetail = () => {
	...
	if (loading) return <div className="min-h-screen bg-[#121212] flex items-center justify-center text-white">로딩 중...</div>;
	if (!session) return <div className="min-h-screen bg-[#121212] flex items-center justify-center text-white">세션을 찾을 수 없습니다.</div>;
			...
			<main className="flex-1 container mx-auto px-6 py-12">
				...
				<div className="bg-[#1f1f1f] rounded-2xl p-8 border border-gray-800 shadow-xl">
					<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
						<div>
							<h1 className="text-3xl font-bold text-[#CAFF33] mb-2">
							...
							<div className="bg-black/20 border border-gray-800 p-6 rounded-xl space-y-4">
								<div className="flex justify-between items-center sm:block sm:space-y-3">
									<div>
										...
										<span className="text-lg font-medium text-gray-200">{new Date(session.attendance.openTime).toLocaleString()}</span>
									</div>
						...
						<div className="flex justify-between items-center mb-6">
							...
							{session.assignment && (
								<p className="text-sm text-gray-400">
									총 <span className="font-bold text-[#CAFF33]">{session.assignment.problemIds.length}</span>문제
								</p>
							)}
						</div>
						...
						{!session.assignment ? (
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 border border-gray-800 rounded-xl p-6 bg-black/20">
								{study?.members.map((member) => {
									...
									return (
										<div key={member.userId} className="bg-black/40 border border-gray-800 p-5 rounded-xl flex flex-col gap-4">
												...
												<div className="flex items-center gap-2">
													...
													<div className="flex flex-col">
														<div className="flex items-center gap-1.5">
															...
															{member.role === 'ADMIN' && (
																<span className="text-[10px] bg-[#CAFF33]/20 text-[#CAFF33] px-1.5 py-0.5 rounded leading-none">Admin</span>
															)}
														</div>
													</div>
												</div>
												<span className="text-lg font-bold text-[#CAFF33] leading-none">{progressPercentage}%</span>
											</div>

											<div className="w-full">
												<div className="w-full bg-gray-800 rounded-full h-2 mb-2">
													<div
														className="bg-[#CAFF33] h-2 rounded-full transition-all duration-500"
